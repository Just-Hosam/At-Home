...
export default function Chores(props) {
	...
	useEffect(() => {
		axios
			...
			.then((res) => {
				...
				const doneChores = res.data.filter((elem) => elem.done);
				const progressRatio = (100 * doneChores.length) / allChores.length;
				setParentChores(allChores);
				setParentDone(doneChores);
				setParentProgress({
	...
	if (parentDone.length === parentChores.length) {
		trophy = <i id="chores-trophy" className="fas fa-trophy fa-3x"></i>;
	} else {
		trophy = null;
	}
